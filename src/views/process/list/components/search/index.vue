<template>
  <div>
    ...
    <el-form-item label="项目名称">
      <el-select v-model="listQuery.projectName" placeholder="请选择项目名称" size="small" filterable clearable style="width: 230px" @change="getList">
        <el-option
...
import { processStructure } from '@/api/process/work-order'
export default {
  ...
  methods: {
    ...
    getProcessNodeList() {
      processStructure({
        ...
        try {
          this.projectNameOptions = this.processStructureValue.tpls[0].form_structure.list.find(item => item.name === '项目名称').options.options
        } catch (error) {
